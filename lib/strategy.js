...
Strategy.prototype.authenticate = function(req) {
  ...
  hawk.server.authenticate(req, this.verify, {}, function(err, credentials, artifacts) {
    if (err && err.isMissing) {
      return this.error(new Error('Missing authentication tokens'));
    }

    if (res) {
      var payload = (!err ? 'Hello ' + credentials.user + ' ' + artifacts.ext : 'Invalid User');

      res.header['Server-Authorization'] = hawk.server.header(credentials, artifacts, {
        payload: payload,
    ...
    if (err && err.message === 'Missing credentials') {
      return this.error(new Error('Invalid authentication tokens'));
    }
    ...
    if (err) {
      return this.error(new Error(err)); // Error String (bad use of errors).
    }
